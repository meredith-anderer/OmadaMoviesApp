//
//  MockMovieSearchExecutor.swift
//  OmadaMoviesApp
//
//  Created by Meredith Anderer on 4/21/25.
//

import Foundation
@testable import OmadaMoviesApp

// TODO: create generalized objecct to aid in stubbing functions in mocks/tests (all mocks, not just this one)
final class MockMovieSearchExecutor: MovieSearchExecutor {
    var queryLog: [(String, Int)] = []
    var result: Result<MovieSearchResponse, Error> = .failure(NSError())

    func search(query: String, page: Int) async throws -> MovieSearchResponse {
        queryLog.append((query, page))
        switch result {
        case .success(let response):
            return response
        case .failure(let error):
            throw error
        }
    }
}
